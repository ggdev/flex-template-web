...
export default function reducer(state = initialState, action = {}) {
  ...
  switch (type) {
    ...
    case ACCOUNT_OPENER_CREATE_SUCCESS:
      return { ...state, createAccountOpenerInProgress: false };
...
export const accountOpenerCreateRequest = () => ({ type: ACCOUNT_OPENER_CREATE_REQUEST });

export const accountOpenerCreateSuccess = response => ({
  type: ACCOUNT_OPENER_CREATE_SUCCESS,
  payload: response,
});

export const accountOpenerCreateError = e => ({
...
const createStripePerson = (personParams, country, stripe) => (dispatch, getState, sdk) => {
  ...
  return stripe
    ...
    .then(response => {
      ...
      if (isAccountOpener) {
        dispatch(accountOpenerCreateRequest());
      } else {
        dispatch(personCreateRequest({ personToken }));
      }

      // const stripePersonParams = {
      //   personToken,
      //   country,
      // };

      return 'sdk.stripePerson.create(stripePersonParams)'; //sdk.stripePerson.create(stripePersonParams);
    })
    .then(response => {
      if (isAccountOpener) {
        dispatch(accountOpenerCreateSuccess());
      } else {
        dispatch(personCreateSuccess({ personToken, response }));
      }
      return response;
    })
    .catch(err => {
      ...
      if (isAccountOpener) {
        dispatch(accountOpenerCreateError(e));
      } else {
        dispatch(personCreateError({ personToken, error: e }));
      }
...
const businessProfileParams = accountData => {
  const { mcc, url } = accountData.businessProfile;
  return {
...
export const createStripeCompanyAccount = (payoutDetails, stripe) => (dispatch, getState, sdk) => {
  ...
  return stripe
    ...
    .then(response => {
      ...
      console.log('stripeAccountParams', stripeAccountParams);
      debugger;
      return sdk.stripeAccount.create(stripeAccountParams, { expand: true });
    })
    .then(response => {
      console.log('sdk.stripeAccount.create', response);
      debugger;
